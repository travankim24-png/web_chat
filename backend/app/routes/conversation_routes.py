...
def create_conv(
    ...
    current_user = Depends(auth.get_current_user)   # ✅ nhận object User thật
):
    conv = crud.create_conversation(
        ...
        payload.is_group,
        payload.member_ids,
        current_user.id  # ✅ chỉ truyền id xuống CRUD
    )
...
def my_convs(
    db_s: Session = Depends(db.get_db),
    current_user = Depends(auth.get_current_user)   # ✅ tương tự ở đây
):
